...
/*   Updated: 2022/05/19 16:43:47 by arudy            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_ray(t_data *data)
{
	...
	float	disH = 100000;
	float	disV = 100000;
	float	hx = data->player->x;
	float	hy = data->player->y;
	float	vx = data->player->x;
	float	vy = data->player->y;
	...
	while (data->player->change && r < 1)
	{
		...
		while (data->ray->dof < data->map_fd->width)
		{
			...
			if (data->ray->mp > 0 && data->ray->mp < data->map_fd->width * data->map_fd->height && data->m[data->ray->mp] == '1')
			{
				hx = data->ray->x;
				hy = data->ray->y;
				disH = dist(data->player->x, data->player->y, hx, hy, data->ray->angle);
				data->ray->dof = data->map_fd->width;
			}
			else
			{
		...
		{
			data->ray->x = (((int)data->player->x >> 6) << 6) + 64;
			data->ray->y = (data->player->x - data->ray->x) * tan_n + data->player->y;
			data->ray->xo = 64;
			data->ray->yo = -data->ray->xo * tan_n;
		}
		...
		while (data->ray->dof < data->map_fd->width)
		{
			...
			if (data->ray->mp > 0 && data->ray->mp < data->map_fd->width * data->map_fd->height && data->m[data->ray->mp] == '1')
			{
				vx = data->ray->x;
				vy = data->ray->y;
				disV = dist(data->player->x, data->player->y, vx, vy, data->ray->angle);
				data->ray->dof = data->map_fd->width;
			}
			else
			{
...
// ====================== AFFICHAGE ======================
		...
		if (disV < disH)
		{
			...
			if (data->ray->old_y && y < data->ray->old_y)
			{
				while (y < data->ray->old_y)
				{
					...
					// data->mlx->addr[(((int)y * data->mlx->line_length) + ((int)x * data->mlx->bpp / 8))] = create_rgb(0, 0, 0);
		...
		if (disV > disH)
		{
			...
			else if (data->ray->old_x)
			{
				while (x < data->ray->old_x)
				{
					data->mlx->addr[(((int)y * data->mlx->line_length) + ((int)x * data->mlx->bpp / 8)) + 0] = (GREEN) & 0xFF;
					data->mlx->addr[(((int)y * data->mlx->line_length) + ((int)x * data->mlx->bpp / 8)) + 1] = ((GREEN) >> 8) & 0xFF;
					data->mlx->addr[(((int)y * data->mlx->line_length) + ((int)x * data->mlx->bpp / 8)) + 2] = ((GREEN) >> 16) & 0xFF;
					data->mlx->addr[(((int)y * data->mlx->line_length) + ((int)x * data->mlx->bpp / 8)) + 3] = ((GREEN) >> 24) & 0xFF;
